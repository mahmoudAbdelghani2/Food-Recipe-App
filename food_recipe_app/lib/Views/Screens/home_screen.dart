...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      body: Row(
        children: [
          ...
          Expanded(
            child: IndexedStack(
              index: isDesktop ? 0 : _currentIndex, // على الديسكتوب نعرض الهوم دايماً
              children: [
                _buildHomeBody(),
                const Center(child: Text('Saved', style: TextStyle(fontSize: 24))),
                const Center(child: Text('Add Recipe', style: TextStyle(fontSize: 24))),
      ...
      // 👉 FAB: يظهر على الموبايل والتابلت فقط
      floatingActionButton: (!isDesktop && _currentIndex == 0)
          ? FloatingActionButton.extended(
        onPressed: () {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Add Recipe Feature')),
          );
        },
    ...
    return Drawer(
      child: ListView(
        ...
        children: [
          ...
          _desktopNavItem(Icons.bookmark_border_rounded, 'Saved', 1, isDrawer: true),
          _desktopNavItem(Icons.add_circle_rounded, 'Add Recipe', 2, isDrawer: true),
          _desktopNavItem(Icons.notifications_none_rounded, 'Notifications', 3, isDrawer: true),
          const Divider(),
          _desktopNavItem(Icons.person_outline_rounded, 'Profile', 4, isDrawer: true),
          _desktopNavItem(Icons.settings_outlined, 'Settings', 5, isDrawer: true),
        ],
      ),
    );
  }

  Widget _desktopNavItem(IconData icon, String label, int index, {bool isDrawer = false}) {
    final isActive = (isDrawer ? _currentIndex : _currentIndex) == index;

    return ListTile(
      leading: Icon(icon,
          color: isActive ? const Color(0xFF1B8A6B) : Colors.black54,
          size: 22),
      title: Text(label,
          style: GoogleFonts.poppins(
            fontSize: 14,
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            icon: const Icon(Icons.logout_rounded, size: 20, color: Colors.black45),
            onPressed: () {
              // Logout logic
            },
          ),
        ...
        children: [
          ...
          Container(
            width: ResponsiveUtils.getValue4<double>(
              context, small: 36, medium: 40, large: 44, xlarge: 48,
            ),
  ...
  Widget _buildFeaturedRecipes() {
    ...
    final cardHeight = ResponsiveUtils.getValue4<double>(
      context, small: 150, medium: 180, large: 200, xlarge: 220,
    );
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.only(
                          bottomLeft: Radius.circular(ResponsiveUtils.borderRadiusLarge(context)),
                          bottomRight: Radius.circular(ResponsiveUtils.borderRadiusLarge(context)),
                        ),
  ...
  Widget _buildRecipeListItem(Map<String, dynamic> recipe, int index) {
    ...
    final imageSize = ResponsiveUtils.getValue4<double>(
      context, small: 75, medium: 90, large: 100, xlarge: 110,
    );
        ...
        children: [
          ClipRRect(
            borderRadius: BorderRadius.only(
              ...
              bottomLeft: Radius.circular(ResponsiveUtils.borderRadius(context)),
            ),
            child: Image.network(
              recipe['image'],
              width: imageSize,
        ...
        child: SizedBox(
          ...
          child: Row(
            ...
            children: [
              ...
              _navItem(Icons.bookmark_border_rounded, 1, 'Saved'),
              const SizedBox(width: 40), // Space for FAB
              _navItem(Icons.notifications_none_rounded, 3, 'Notifications'),
              _navItem(Icons.person_outline_rounded, 4, 'Profile'),
            ],
          ),
        ),
      ),
    ...
    return GestureDetector(
      ...
      child: Column(
        ...
        children: [
          Icon(
            icon,
            color: isActive ? const Color(0xFF1B8A6B) : Colors.black38,
            size: ResponsiveUtils.iconSize(context),
          ),
          const SizedBox(height: 2),
          Text(
